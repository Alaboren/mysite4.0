(function(){
  ...
  async function loadStore(){
    ...
    const url = 'data/pinyintable.json';
  ...
  async function loadToneMap(){
    if (TONE_MAP) return TONE_MAP;
    TONE_MAP = await loadJson('data/pinyin-tone-map.json', {});
    return TONE_MAP;
  }

  async function loadArabicToPinyin(){
    if (ARABIC_TO_PINYIN) return ARABIC_TO_PINYIN;

    const mapping = await loadJson('data/pinyin-mapping.json', {});
    const reverse = {};

    Object.keys(mapping || {}).forEach(initial => {
      const finals = mapping[initial] || {};
      Object.keys(finals).forEach(final => {
        const arabic = String(finals[final] || '').trim();
        if (!arabic) return;

        const canonical = canonicalPinyin(initial, final);
        if (!reverse[arabic]) reverse[arabic] = [];
        if (!reverse[arabic].includes(canonical)) reverse[arabic].push(canonical);
      });
    });

    ARABIC_TO_PINYIN = reverse;
    return ARABIC_TO_PINYIN;
  }

  function canonicalPinyin(initial, final){
    const i = String(initial || '').trim();
    let f = String(final || '').trim();

    if (i === 'a' || i === 'o' || i === 'e') return f;

    if (i === 'y'){
      const yMap = {
        i: 'yi', ia: 'ya', iao: 'yao', ian: 'yan', iang: 'yang',
        ie: 'ye', in: 'yin', ing: 'ying', iong: 'yong', iu: 'you',
        u: 'yu', uan: 'yuan', ue: 'yue', un: 'yun'
      };
      return yMap[f] || ('y' + f);
    }
    ...
    if (i === 'w'){
      const wMap = {
        ...
        ui: 'wei', un: 'wen', eng: 'weng', uo: 'wo'
      };
      return wMap[f] || ('w' + f);
    }

    if ((i === 'b' || i === 'p' || i === 'm' || i === 'f') && f === 'uo') {
      f = 'o';
    }

    if ((i === 'n' || i === 'l') && f.charAt(0) === 'v') {
      f = 'ü' + f.slice(1);
    }
  ...
  function syllablesForCellText(text, arabicLookup){
    const value = String(text || '').trim();
    if (!value) return [];

    if (/^[a-zü:]+$/i.test(value)) return [normalizePinyinText(value)];
    return arabicLookup[value] || [];
  }

  async function applyToneAvailability(tone){
    const toneMap = await loadToneMap();
    const arabicLookup = await loadArabicToPinyin();

    document.querySelectorAll('.pinyin-grid-table tbody td').forEach(cell => {
      const original = String(cell.textContent || '').trim();
      if (!original) return;

      const syllables = syllablesForCellText(original, arabicLookup);
      if (!syllables.length) return;

      const allowed = syllables.some(syllable => {
        const tones = toneMap[syllable];
        return Array.isArray(tones) && tones.includes(Number(tone));
      });

      cell.dataset.pinyinKeys = syllables.join(' ');
      cell.classList.toggle('tone-unavailable', !allowed);
      cell.removeAttribute('title');

      if (!allowed) {
        cell.dataset.originalText = original;
        cell.dataset.unavailableReason = `${syllables.join(' / ')} tone ${tone}`;
        cell.textContent = '';
      }
    });
  }
  ...
  async function render(){
    ...
    renderInFlight = (async () => {
      ...
      await applyToneAvailability(tone);
      applyColumnView();
    })();
  ...
  function penguinBurstForTone(tone){
    ...
    const toneToSrc = {
      1: 'assets/img/first.png',
      2: 'assets/img/second.png',
      3: 'assets/img/third.png',
      4: 'assets/img/fourth.png'
    };
