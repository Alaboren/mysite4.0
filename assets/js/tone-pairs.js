...
async function loadCards() {
    try {
        ...
        if (!response.ok) throw new Error('Failed to load data/cards.json');
...
for (let r = 0; r < 5; r++) {
    for (let c = 0; c < 6; c++) {
        ...
        if (r === 0) {
            ...
            cell.addEventListener('pointerup', handleCellPointerUp);
        }
    ...
    wrapper.innerHTML = `
        <div class="card-inner">
            <div class="card-front">
                <img class="card-logo" src="assets/img/icon-512.png" alt="Alaboren" draggable="false" />
            </div>
            <div class="card-back">
...
function handleDrop(e) {
    e.preventDefault();
    checkAnswer(this.dataset.cellId);
}

function handleCellPointerUp() {
    if (!stackElement || !stackElement.querySelector('.card-wrapper')) return;
    checkAnswer(this.dataset.cellId);
}

function checkAnswer(targetCellId) {
    if (currentIndex < 0 || currentIndex >= cardsData.length || !targetCellId) return;
    const currentCard = cardsData[currentIndex];
    if (targetCellId === currentCard.correctCell) handleSuccess();
    else handleFailure();
}
